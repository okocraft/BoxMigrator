...
public class MasterTable {
    ...
    public void migrate(@NotNull BoxUser user, int id) {
        ...
        if (player != null && playerMap.isLoaded(player)) {
            ...
            autoStoreSetting = getAutoStoreSetting(player);
        } else {
    ...
    private @NotNull AutoStoreSetting getAutoStoreSetting(@NotNull Player player) {
        try {
            return AutoStoreSettingContainer.INSTANCE.get(player);
        } catch (IllegalStateException e) {
            return new AutoStoreSetting(player.getUniqueId());
        }
    }
